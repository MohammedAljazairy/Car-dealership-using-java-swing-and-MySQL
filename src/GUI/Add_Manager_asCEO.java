...
public class Add_Manager_asCEO extends JFrame implements ActionListener{
...
depid.setBounds(30,460,520,25);   
panel.add(depid);                 
depid_i = new JTextField(15);     
depid_i.setBounds(100,460 , 140, 25);
panel.add(depid_i);
Branchid =new JLabel("Branch Id ");     
Branchid.setBounds(30,520,400,25);        
panel.add(Branchid);                     
Branchid_i = new JTextField(15);
Branchid_i.setBounds(100,520, 140, 25); 
panel.add(Branchid_i);
pass=new JLabel("password");
pass.setBounds(30,580,400,25);  
panel.add(pass); 
pass_t = new JTextField(15);
pass_t.setBounds(100,580, 140, 25); 
panel.add(pass_t);
...
frame.setVisible(true);
	...
	int contract=Integer.parseInt(contract_T.getText()); 
	...
	JOptionPane.showMessageDialog(this,"The manager has been created"); 
	
	Database.Connection_To_Database_insert_Manager(nid,fname,lname,email,hiredate,contract,salary,bonus,departmentid,branchid,pass);
	...
	Database.Connection_To_Database_insert_Phone_employee(phonenum);
}
}
	}